...
import static protocol.DataPackage.*;
...
public class BaseSocket {
            ...
            try {
                ...
                if (mTimeoutThread == null) {
                    ...
                    mTimeoutHandler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            if (mExit == false && isConnected() == false) {
                                send(DATA_PROTO_CMD_BEATHEART, new BeatHeart(), new SocketCallback());
                            }
    ...
    public Thread recvThread = new Thread() {
        ...
        public void run() {
            short pkglen = 0;
            byte[] byteBuffer = new byte[MAX_DATA_PACKAGE_SIZE];
            while (mExit == false) {
                ...
                try {
                    // 默认大端方式读取
                    pkglen = in.readShort();
                    if (pkglen > MAX_DATA_PACKAGE_SIZE || pkglen < BaseData.DATA_PACKAGE_HEAD_SIZE) {
                ...
                BaseData baseData = convertBytesToObject(ByteBuffer.wrap(byteBuffer, 0, pkglen));
                if (baseData == null) {
    ...
    public Thread sendThread = new Thread() {
        ...
        public void run() {
            ...
            while (mExit == false) {
                ...
                try {
                    out.write(convertObjectToBytes(baseData));
                } catch (Exception e) {
