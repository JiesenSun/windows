...
func SaveUserMsg(userMsg *UserMsgItem) error {
	g_userMsgMgr.msgMap.SetByTimeout(msgKey(userMsg.ToUid, userMsg.MsgId), userMsg, time.Minute*2)
	return nil
}
...
type OfflineMsgIn struct {
	Uid uint64
}
type OfflineMsgOut struct {
	Msgs []*UserMsgItem
}

func (OfflineMsg) Get(in *OfflineMsgIn, out *OfflineMsgOut) error {
	out.Msgs, _ = GetUserMsgList(in.Uid)
