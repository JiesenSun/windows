...
func GetUserTeamMsgList(uid uint64) ([]*TextTeamMsg, error) {
	...
	for _, msgId := range msgIdList {
		...
		if err := dataPackage.Unpackage(msgBuf); err != nil {
			return msgList, errors.As(err, uid)
		}
		msg := new(TextTeamMsg)
		if err := json.Unmarshal(dataPackage.BodyData(), msg); err != nil {
			return msgList, errors.As(err, string(dataPackage.BodyData()))
		}
